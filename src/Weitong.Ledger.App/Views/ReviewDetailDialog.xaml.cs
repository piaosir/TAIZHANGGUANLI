...
{
    /// <summary>用户在弹窗内的选择："confirm" / "reject" / null（仅查看）。</summary>
    ...
    public ReviewDetailDialog(ReviewDetail detail)
    {
        ...
        RejectBtn.Visibility = detail.IsIncoming && detail.CanReject ? Visibility.Visible : Visibility.Collapsed;
    }

    private void OnConfirm(object sender, RoutedEventArgs e) { Action = "confirm"; DialogResult = true; }
    private void OnReject(object sender, RoutedEventArgs e) { Action = "reject"; DialogResult = true; }
